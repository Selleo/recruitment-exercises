...
1. Please write a script called `dice_roller.rb`. When launched using shell (`ruby dice_roller.rb`) it should display a randomly generated number from 1-20 range. Imagine this as rolling a twenty sided dice (k20).
2. In this iteration, running the `dice_roller.rb` should return 5 randomly generated numbers (five dice rolls).
...
7. In this iteration, running the `dice_roller.rb` script should display the following: "Your randomly generated set: #{results}. Are you happy?" and the input prompt. Entering anything other than "YES!" restarts the script.
